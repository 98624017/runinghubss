...
describe('HistoryPage', () => {
  it('应支持读取历史、按应用和状态筛选，并提供再次生成入口', async () => {
    ...
    expect((await screen.findAllByRole('heading', { name: '任务记录' })).length).toBeGreaterThan(0);
    expect(screen.getByText('TASK-1001')).toBeInTheDocument();
    expect(screen.getByRole('img', { name: '一键彩平 输出结果 1' })).toHaveAttribute(
      'src',

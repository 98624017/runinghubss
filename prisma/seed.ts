...
async function main() {
  ...
  // ============================================
  // 四个默认 AI 应用（仅首次 seed 时创建，之后可自由删除）
  // ============================================
  const alreadySeeded = await prisma.systemConfig.findUnique({
    where: { key: "appsSeeded" },
  });

  if (alreadySeeded) {
    console.log("Apps already seeded, skipping. (Delete 'appsSeeded' from system_config to re-seed apps)");
  } else {
    await prisma.$transaction(async (tx) => {
      // 1. 一键彩平
      const app1 = await tx.aiApp.upsert({
        ...
        create: {
          ...
          icon: "palette",
          coverImage: "/images/defaults/floor-plan.webp",
      ...
      // 2. 外观迁移
      const app2 = await tx.aiApp.upsert({
        ...
        create: {
          ...
          icon: "repeat",
          coverImage: "/images/defaults/style-transfer.webp",
      ...
      // 3. 平面转效果
      const app3 = await tx.aiApp.upsert({
        ...
        create: {
          ...
          icon: "layers",
          coverImage: "/images/defaults/rendering.webp",
      ...
      // 4. 毛坯转效果
      const app4 = await tx.aiApp.upsert({
        ...
        create: {
          ...
          icon: "home",
          coverImage: "/images/defaults/renovation.webp",
      ...
      // 标记应用已初始化，后续 seed 不再重复创建
      await tx.systemConfig.create({
        data: { key: "appsSeeded", value: "true" },
      });
    });
    console.log("Default apps seeded.");
  }
...
async function seedFields(
  ...
  await tx.aiAppField.deleteMany({ where: { appId } });
  await tx.aiAppField.createMany({
    data: fields.map((f) => ({ appId, ...f })),
  });
}
